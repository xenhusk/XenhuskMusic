"""
Vercel-compatible Music Classification Service
...
# Thread pool for parallel processing (optimized for Vercel)
MAX_WORKERS = min(2, mp.cpu_count())  # Limit to 2 workers for Vercel
thread_pool = ThreadPoolExecutor(max_workers=MAX_WORKERS)

# Processing queue for batch operations
processing_queue = Queue(maxsize=50)  # Smaller queue for serverless
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_song):
                try:
                    result = future.result(timeout=15)  # Shorter timeout for Vercel
...
def root():
    ...
    return jsonify({
        'service': 'Music Classification Service (Vercel)',
        'version': '1.0.0',
...
def batch_classify():
    ...
    try:
        ...
        if len(songs) > 50:  # Smaller limit for Vercel
            raise BadRequest("Maximum 50 songs per batch for Vercel")
...
def model_info():
    ...
    return jsonify({
        ...
        'performance': {
            ...
            'optimization_level': 'vercel_serverless_optimized'
        },
        'platform': 'Vercel'
    })
...
def performance_stats():
    ...
    return jsonify({
        'service_status': 'vercel_serverless_optimized',
        'optimization_level': 'high_performance_serverless',
        ...
        'vercel_specs': {
            ...
            'memory': '1024MB RAM',
            'cpu': '2 vCPU',
            'optimization_strategy': 'serverless_multithreading',
            'max_concurrent_requests': MAX_WORKERS,
            'recommended_batch_size': '10-25 songs',
...
if __name__ == '__main__':
    logger.info("🚀 Starting Vercel-Optimized Music Classification Service...")
    ...
    if load_model():
        ...
        logger.info("   POST /batch_classify - Classify multiple songs (up to 50)")
        logger.info("   GET  /model_info - Get model information")
        logger.info("   GET  /performance - Performance statistics")
        logger.info("   GET  / - Service information")
        logger.info(f"🔧 Optimization: {MAX_WORKERS} workers, Vercel serverless")
        logger.info("💾 Memory optimized for Vercel (1024MB)")
